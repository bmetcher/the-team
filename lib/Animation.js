...
export class MovingStamp extends Stamp {
    ...
    update() {
        super.update();
        // Stop progressing animation if not being drawn
        if ((this.#tad.frameCount - this.#lastFrameDrawn) <= 1) {
            this.#progressFrame();
        }
    }
    ...
    _draw(x = this.x, y = this.y, rotation = this.rotation) {
        ...
        ctx.translate(x, y);
        ctx.rotate(this.#tad.math.degreeToRadian(rotation % 360));
        ctx.translate(this.offset.x, this.offset.y);
        ctx.scale(scaleX, scaleY);

        const frame = this.#frames[this.frame];
        //@ts-expect-error
        ctx.drawImage(frame.raw, -frame.w/2, -frame.h/2, frame.w, frame.h);

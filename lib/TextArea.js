...
export class TextArea extends ShapedAssetEntity {
	...
	#handleClick() {
		...
		if (this.#focused) {
			if (this.#tad.mouse.leftDown) {
				...
				const currEndIndex = this.#getIndexFromCoords(this.#tad.mouse.x);
	...
	#drawSelection(start, end) {
		...
		this.#tad.colour.fill = "rgba(0, 115, 130, 0.3)";
		this.#tad.colour.stroke = "transparent";
	...
	#handleKeyboardInput() {
		...
		//@ts-expect-error
		this.#tad.keys.activeBuffer._forEach((/** @type {{ key: string; type: string; }} */ event) => {

...
export class Camera {
     ...
     */
    worldToScreen(x, y) {
        const temp = Vector.temp(x, y);
        temp.subtract(this.#position);
        temp.rotate(-this.#rotation);
        temp.multiply(this.#scale);
        temp.x += this.#tad.w / 2;
        temp.y += this.#tad.h / 2;
        return new Vector(temp.x, temp.y);
    }
    ...
    draw() {
        if (
            this.#tad.debug &&
            (this.#position.x !== this.#tad.w / 2 ||
                this.#position.y !== this.#tad.h / 2)
        ) {

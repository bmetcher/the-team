...
export class Group extends Array {
    ...
    /**
     * @type {string[]} The #approvedColours circular buffer.
     * Initialised in Group constructor.
     */
    static #approvedColours = null;
    ...
    constructor(...elements) {
        ...
        if (Group.#approvedColours === null) {
            Group.#approvedColours = [
                Paint.palegreen,
                Paint.green,
                Paint.paleblue,
                Paint.blue,
        ...
        this.colour = Group.#approvedColours[Group.#approvedColoursIdx];
        Group.#approvedColoursIdx =
            (Group.#approvedColoursIdx + 1) % Group.#approvedColours.length;
            ...
            if (isCollider) {
                if (!elements[i].groupsOwnedBy.includes(this.id)) {
                    if (elements[i].hadColourAssigned === false) {
                        elements[i]._fillWithoutTrackingAssignment = this.colour;
                    }
    ...
    overlaps(collideableEntity) {
        if (!this.tad) {
            throw new Error(
                "Tad is not bound to this group! Was it made via $.makeGroup?"
            );
        }
        ...
        const thisValidGroupType =
            this.type === "Collider" || this.type === "empty";
        if (collideableEntity === this && thisValidGroupType) {
            return this.tad
                .getCollisionManager()
                .collidesOrOverlapsGroupSelfToSelf(this, this.id, true);
        }

        const otherValidGroupType =
            collideableEntity.type === "Collider" ||
            collideableEntity.type === "empty";
        if (collideableEntity instanceof Group && otherValidGroupType) {
            return this.tad
                .getCollisionManager()
        ...
        const msg =
            ErrorMsgManager.colliderCheckFailed(collideableEntity) || "";
        throw new Error(
    ...
    //@ts-expect-error
    map() {
        throw new Error(`Groups do not support using .map() both for technical and educational reasons look into a for or for of loop instead :)`);
    }

    //@ts-expect-error
    filter() {
        throw new Error(`Groups do not support using .filter() both for technical and educational reasons look into a for or for of loop instead :)`);
    }
    
    reduce() {
        throw new Error(`Groups do not support using .reduce() both for technical and educational reasons look into a for or for of loop instead :)`);
    }
}
